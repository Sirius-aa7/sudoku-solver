...
public class SudokuBoard extends View {
    ...
    public SudokuBoard(Context context, @Nullable AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,R.styleable.SudokuBoard
                ,0,0);//declaring array for storing attrs

        try {
            boardColor= a.getInteger(R.styleable.SudokuBoard_boardColor,0);//assigning boardColor
            ...
            a.recycle();//clearing all the attrs
        }
    }
    ...
    protected void onMeasure(int widthMeasuredSpec, int heightMeasuredSpec){
        ...
        int dimension = Math.min(MeasureSpec.getSize(widthMeasuredSpec),MeasureSpec.getSize(heightMeasuredSpec));
        //finding the smaller among the 2
        cellSize = dimension / 9;

        setMeasuredDimension(dimension,dimension);// assigning both ht and width as the smaller one
    }

    @Override
    protected void onDraw(Canvas canvas){
        boardColorPaint.setStyle(Paint.Style.STROKE);//this and lower line act like a paintbrush
        boardColorPaint.setStrokeWidth(16);
        boardColorPaint.setColor(boardColor);// like a paint bucket
        boardColorPaint.setAntiAlias(true);

        cellFillColorPaint.setStyle(Paint.Style.FILL);// JUST MAKING THE  ABOVE PAINTING POSS FOR THESE 2 ATTRS
    ...
    private void colorCell(Canvas canvas, int r, int c){
        if (solver.getSelectedColumn()!=-1 && solver.getSelectedRow()!=-1){
            ...
            canvas.drawRect( (c-1)*cellSize,(r-1)*cellSize,cellSize*c,cellSize*r,
                    cellsHighlightColorPaint);
        }
        invalidate();
    }
    ...
    private void drawBoard(Canvas canvas){
        ...
        for(int r=0;r<10;r++){
            ...
            //we use getWidth in both places as its a square
        }
